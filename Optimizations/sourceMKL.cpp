...
float **multWithoutPadding_MKL(float **Input, int sizeInput, float **Kernel, int sizeKernel) {
	...
	newA = (float *)mkl_malloc(rows*columns * sizeof(float), 64);
	newB = (float *)mkl_malloc(columns * 1 * sizeof(float), 64);
	...
	float *newC;
	newC = (float *)mkl_malloc(rows * 1 * sizeof(float), 64);
...
//	int padding = (sizeKernel - 1) / 2;
	...
	float **Answer;
	Answer = multWithoutPadding_MKL(newMatrixIn, newSize, Kernel, sizeKernel);
